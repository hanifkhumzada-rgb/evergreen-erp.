...
export function createAdminClient() {
  ...
  // Support both Supabase's modern Secret key name and the legacy
  // service_role key. Both remain server-only.
  const serverSecret = process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!serverSecret) throw new Error("Missing server-side Supabase secret key");
