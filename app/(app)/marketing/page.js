...
export default async function MarketingPage() {
  ...
  const [{ count: active }, { count: overdue }, { count: monthly }, { count: cashCustomers }] = await Promise.all([
    ...
    supabase.from("customers").select("id", { count: "exact", head: true }).eq("is_active", true).ilike("payment_frequency", "cash"),
  ]);
  const segments = [
