...
export default async function NotificationsPage({ searchParams }) {
  ...
  const notifications = q
    ? (allNotifications || []).filter((n) => n.title?.toLowerCase().includes(q) || n.message?.toLowerCase().includes(q))
    : (allNotifications || []);
  ...
  return (
    <div>
      ...
      <form className="no-print flex flex-wrap gap-2.5 mb-4 items-center" action="/notifications">
        ...
        {q && <a href="/notifications" className="text-xs text-slate hover:text-aqua">Clear</a>}
      </form>
