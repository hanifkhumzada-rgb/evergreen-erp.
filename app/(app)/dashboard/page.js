...
export default async function DashboardPage() {
  ...
  const activeCustomersRes = await supabase.from("customers").select("id", { count: "exact", head: true }).eq("is_active", true);
  ...
  const { data: overdueCustomers } = await supabase.from("v_customer_balance").select("customer_id, name, balance").gt("balance", 1000).order("balance", { ascending: false }).limit(5);
