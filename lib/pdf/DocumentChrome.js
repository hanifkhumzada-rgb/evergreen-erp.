...
const styles = StyleSheet.create({
  ...
  logo: { width: 34, height: 34, borderRadius: 7, marginRight: 9 },
  // Letterhead mark: bigger + letter-spaced than ordinary body/heading text
  // elsewhere in these documents, so the company name reads as a brand
  // mark at a glance rather than just another label on the page.
  brandName: { fontSize: 16, fontFamily: "Helvetica-Bold", color: COLORS.navy, letterSpacing: 0.3 },
  brandSub: { fontSize: 7, color: COLORS.slate, marginTop: 2, letterSpacing: 0.2 },
  brandAddress: { fontSize: 7, color: COLORS.slate, marginTop: 1, maxWidth: 220 },
...
export function PdfShell({ title, meta, businessName, address, children }) {
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.headerRow}>
          <View style={styles.brandRow}>
            ...
            <View>
              <Text style={styles.brandName}>{businessName || "Evergreen Water"}</Text>
              <Text style={styles.brandSub}>Digital HQ</Text>
              {address ? <Text style={styles.brandAddress}>{address}</Text> : null}
            </View>
          </View>
