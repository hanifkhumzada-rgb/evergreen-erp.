...
export default function InvoiceDocument({ invoice, customer, items, paid, previousBalance, newBalance, businessName, address }) {
  return (
    <PdfShell title="Invoice" meta={`${invoice.invoice_no} · ${fmtDate(invoice.invoice_date)} · ${STATUS_LABEL[invoice.status] || invoice.status}`} businessName={businessName} address={address}>
