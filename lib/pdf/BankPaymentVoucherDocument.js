...
export default function BankPaymentVoucherDocument({ record, module, date, party, transactionType, chequeRef, entry, lines, currency, businessName, address }) {
  const total = lines.reduce((a, l) => a + Number(l.debit), 0);
  return (
    <PdfShell title="Bank Payment Voucher" meta={`${record.bpv_no} · ${fmtDate(date)}`} businessName={businessName} address={address}>
      <VoucherInfo items={[
