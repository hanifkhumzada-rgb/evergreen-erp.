...
export default function JournalVoucherDocument({ entry, lines, branding }) {
  const total = lines.reduce((a, l) => a + Number(l.debit), 0);
  return (
    <PdfShell title="Journal Voucher" meta={`${entry.entry_no} · ${fmtDate(entry.entry_date)}`} branding={branding}>
      <VoucherInfo items={[
