...
export default async function InvoicesPage({ searchParams }) {
  ...
  const totalBilled = allRows.reduce((a, s) => a + Number(s.net_amount), 0);
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3.5 mb-5">
        <KPI label="TODAY'S INVOICES" value={todaysInvoices.length} tone="navy" sub={pkr(todaysInvoices.reduce((a, s) => a + Number(s.net_amount), 0))} />
        <KPI label="THIS MONTH" value={monthInvoices.length} tone="aqua" sub={pkr(monthInvoices.reduce((a, s) => a + Number(s.net_amount), 0))} />
      ...
      <div className="overflow-x-auto border border-line rounded-2xl">
        <table className="w-full text-[13.5px] border-collapse">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-slate">No invoices match.</td></tr>}
            {rows.map((s) => (
