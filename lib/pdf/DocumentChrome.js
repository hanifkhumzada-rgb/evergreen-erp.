...
const styles = StyleSheet.create({
  ...
  // Invoice-only header treatment (`accent` prop) — a heavier aqua
  // underline in place of the shared navy one, plus the full-bleed top
  // band below, so an invoice reads as visually distinct from the generic
  // report/voucher letterhead at a glance.
  headerRowAccent: { borderBottomWidth: 3, borderBottomColor: COLORS.aqua },
  accentTopBar: { position: "absolute", top: 0, left: 0, right: 0, height: 5, backgroundColor: COLORS.aqua },
  // Faint diagonal "EW" corner mark — painted before the header/content so
  // everything else prints on top of it. Kept to a whisper (4% opacity)
  // so it never competes with the actual document content or print/scan
  // legibility.
  watermark: { position: "absolute", right: -26, bottom: 96, fontSize: 190, fontFamily: "Helvetica-Bold", color: COLORS.navy, opacity: 0.04, transform: "rotate(-28deg)" },
  headerLeft: { width: 64 },
...
export function PdfShell({ title, meta, branding, children, accent = false }) {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        {accent && <View style={styles.accentTopBar} fixed />}
        {accent && <Text style={styles.watermark} fixed>EW</Text>}
        <View style={[styles.headerRow, accent && styles.headerRowAccent]}>
