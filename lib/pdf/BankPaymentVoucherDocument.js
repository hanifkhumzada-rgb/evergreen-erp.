...
export default function BankPaymentVoucherDocument({ record, module, date, party, transactionType, chequeRef, entry, lines, currency, branding }) {
  const total = lines.reduce((a, l) => a + Number(l.debit), 0);
  return (
    <PdfShell title="Bank Payment Voucher" meta={`${record.bpv_no} · ${fmtDate(date)}`} branding={branding}>
      <VoucherInfo items={[
