...
export default function InvoiceDocument({ invoice, customer, items, paid, previousBalance, newBalance, branding }) {
  ...
  return (
    <PdfShell
      ...
      accent
    >
      {/* Left accent rail + heavier name treatment is invoice-only — every
          other document (statement, vouchers, reports) keeps the plain
          foam card, so an invoice reads as the more "billable" document
          at a glance, not just another report with a table under it. */}
      <View style={{ backgroundColor: COLORS.foam, borderRadius: 8, padding: 12, paddingLeft: 14, marginBottom: 16, flexDirection: "row", justifyContent: "space-between", borderLeftWidth: 3, borderLeftColor: COLORS.aqua }}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 16.5, fontFamily: "Helvetica-Bold", color: COLORS.navy, letterSpacing: 0.2 }}>{customer?.name}</Text>
      ...
      <View style={{ alignItems: "flex-end", marginBottom: 18 }}>
        <View style={{ width: 230 }}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 10, paddingHorizontal: 12, marginTop: 8, backgroundColor: COLORS.navy, borderRadius: 6, borderLeftWidth: 3, borderLeftColor: COLORS.aqua }}>
            <Text style={{ fontSize: 9, fontFamily: "Helvetica-Bold", color: "#fff", letterSpacing: 0.4, textTransform: "uppercase" }}>Total Payable</Text>
            <Text style={{ fontSize: 15, fontFamily: "Helvetica-Bold", color: "#fff" }}>{pkr(newBalance)}</Text>
          </View>
        </View>
      </View>
