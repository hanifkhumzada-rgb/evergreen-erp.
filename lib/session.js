...
export const getCurrentProfile = cache(async () => {
  ...
  // Keep server-verified identity here. React cache() below still guarantees
  // this runs only once per render request even when layout and page both use it.
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { supabase, user: null, profile: null, businessId: null, roleKey: null };
  const { data: workspace, error: profileError } = await supabase.rpc("fn_erp_workspace_context");
  if (profileError) {
    console.error("[session] workspace lookup failed", { code: profileError.code });
    throw new Error("Workspace could not load. Please retry.");
  }
  ...
  return { supabase, user, profile, businessId: profile?.business_id ?? null, roleKey,
    permissions: workspace?.permissions || [], unreadNotifications: Number(workspace?.unread_notifications || 0) };
});
